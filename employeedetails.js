...
const employees2 = employees.filter((emp) => {
  return emp > 10000;
});
...
function getEmployeeIds(employees, targetSalary) {
  if (employees?.length && targetSalary) {
    return employees
      .filter((oEmployee) => oEmployee.salary >= targetSalary)
      .map((oEmployee) => oEmployee.id);
  }
  return [];

  return employees?.length && targetSalary
    ? employees
        .filter((oEmployee) => oEmployee.salary >= targetSalary)
        .map((oEmployee) => oEmployee.id)
    : [];
